...
@section('title')
    <!--begin::Heading-->
    <h1 class="text-dark fw-bold my-0 fs-2">{{ucfirst(__('stockout.create'))}}</h1>
    <!--end::Heading-->
    <!--begin::Breadcrumb-->
    <ul class="breadcrumb fw-semibold fs-base my-1">
        <li class="breadcrumb-item text-muted">{{__('stockout.stockout')}}</li>
        <li class="breadcrumb-item text-muted"><a href="{{route('stock-out.index')}}">{{__('stockout.list')}}</a></li>
        <li class="breadcrumb-item text-dark">{{__('stockout.add')}}</li>
    </ul>
    <!--end::Breadcrumb-->
@endsection
            ...
            <form action="{{route('stock-out.store')}}" method="POST">
                ...
                <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10 mb-5">
                    <div class="col-12 my-5 mb-2 input-group flex-nowrap">
                        ...
                        <select name="business_location_id" id="business_location_id" class="form-select rounded-0" data-kt-select2="true"  data-placeholder="{{__('stockout.placeholder_location')}}">
                            <option selected></option>
                            @foreach ($locations as $location)
                                <option {{$location->id == \Illuminate\Support\Facades\Auth::user()->default_location_id ? 'selected' : ''}} @selected($location->id == old('business_location_id')) value="{{$location->id}}">{{$location->name}}</option>
                            @endforeach
                        </select>
                        <button type="button" class="input-group-text "  data-bs-toggle="tooltip" data-bs-custom-class="tooltip" data-bs-placement="top" data-bs-html="true" title="<span class='text-primary-emphasis'>{{__('stockout.location_usage')}}</span>">
                                ...
                                <div class="mb-7 mt-3 col-12 col-md-4">
                                    ...
                                    <div class="input-group flex-nowrap">
                                        ...
                                        <select name="customers" class="form-select form-select-sm  fw-bold rounded-start-0" id="customerOption" data-kt-select2="true"
                                                data-hide-search="false" data-placeholder="{{__('stockout.placeholder_customer')}}"
...
{{--                                </div>--}}
                                <!--begin::Input group-->
                                <div class="mb-7 mt-3 col-12 col-md-4">
                                    <label class="form-label fs-6 fw-semibold required">{{__('stockout.stockout_person')}}</label>
                                    <div class="input-group flex-nowrap">
                                        <div class="input-group-text">
                                            <i class="fa-solid fa-user text-muted"></i>
                                        </div>
                                        <select name="stockout_person" class="form-select form-select-sm fw-bold rounded-start-0"
                                                data-kt-select2="true" data-hide-search="false"
                                                data-placeholder="{{__('stockout.placeholder_person')}}" data-allow-clear="true"
                                                data-kt-user-table-filter="role" data-hide-search="true">
                                            <option></option>
                                ...
                                <div class="mb-7 mt-3 col-12 col-md-6">
                                    <label class="form-label fs-6">
                                        {{__('stockout.sale_order')}}
                                    </label>
                                    <input name="sale_voucher_no" class="form-control form-control-sm d-flex align-items-center" placeholder="{{__('stockout.placeholder_sale_order')}}" value="" id="tagify_purchase_ordder" />
                            ...
                            <div class="table-responsive">
                                <table class="table align-middle table-row-dashed fs-6 gy-5 mt-6" id="stockout_table">
                                    ...
                                    <tr class="text-start text-primary fw-bold fs-7 text-uppercase gs-0 ">
                                        ...
                                        <th class="min-w-200px">{{__('stockout.product')}}</th>
                                        <th class="min-w-100px">{{__('stockout.ordered')}}</th>
                                        <th class="min-w-100px">{{__('stockout.delivered')}}</th>
                                        <th class="min-w-100px">{{__('stockout.out')}}</th>
                                        <th class="min-w-125px">{{__('stockout.uom')}}</th>
                                        <th class="min-w-200px">{{__('stockout.remark')}}</th>
                    ...
                    <div class="col-12 text-center mt-2 mb-5">
                        <button type="submit" class="btn btn-primary btn-lg save_btn">{{__('stockout.save')}}</button>
                    </div>
            </form>
        </div>
        <!--end::Container-->
