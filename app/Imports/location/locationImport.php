    ...
    WithBatchInserts
{
    ...
    public function collection(Collection $rows)
    {
        try {
            ...
            foreach ($chunkedRows as  $rows) {
                foreach ($rows as  $row) {
                    $locationData=$this->prepareLocationData($row);
                    $locationAddress = $this->prepareLocationAddress($row);
    ...
    public function prepareLocationData($data){
        return [
            ...
            'is_acitve' => 1,
            'allow_purchase_order' => $data['purchase_order'] ?? 0,
            'allow_sale_order' => $data['sale_order'] ?? 0,
            'parent_location_id' => $this->getParentLocationId($data['parent_location']),
            'location_type' => $this->getLocationType($data['location_type']),
    ...
    public function prepareLocationAddress($data)
    {
        return [
            'mobile' => $data['mobile'],
            'alternate_number' => $data['alt_number'],
            'email' => $data['email'] ?? null,
            'address' => $data['address'],
            'country' => $data['country'],
            'state' => $data['state'],
            'city' => $data['city'],
            'zip_postal_code' => $data['zip_code'],
        ];
    }
    public function getLocationType($typeName){
        $types = ['supplier location', 'customer location', 'view', 'internal location', 'transit location'];
        return array_search(strtolower($typeName),$types) + 1;
    }

    public function getParentLocationId($name){
        $id = optional(businessLocation::where('name', $name)->select('id')->first())->id;
        return $id;
    }

    public function getPricelistId($name)
    {
        $id = optional(PriceLists::where('name', $name)->select('id')->first())->id;
        return $id;
    }
    public function getInvoiceTemplayeId($name)
    {
        $id = optional(InvoiceTemplate::where('name', $name)->select('id')->first())->id;
        return $id;
    }
