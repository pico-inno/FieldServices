...
<html lang="en">
    ...
    <!--begin::Body-->
    <body >
         <div class="container-fluid p-0 position-fixed" style="height: 100vh" >
            ...
            <div class="d-flex flex-row position-lg-relative" style="height: 94vh">
                <div class="col-lg-3 col-6 nav-fill card rounded-0 position-relative" style="z-index: 99999" id="sidebar" style="height: 100%;">
                    <div class="d-flex flex-column px-5 py-3 shadow-sm mb-5">
                        <div class="col-12 text-center p-1">
                            <h4 class="text-primary-emphasis" id="detailHeader">Please Select A Table</h4>
                        </div>
                        <div class="col-12 d-flex justify-content-between">
                            <div class="">
                                <h2 class=" fs-6 fw-bold text-gray-700">Products</h2>
                            </div>
                            <div class="">
                                <h2 class=" fs-6 fw-bold text-gray-700">Qty</h2>
                            </div>
                        </div>
                    ...
                    <div class="w-100  position-absolute py-3  bottom-sm-0" style="bottom: 5px;padding-bottom: 20px">
                        <div class="col-12 p-2">
                            <div class="col-12 d-flex justify-content-around  align-items-center">
                                ...
                                <div class="col-2">
                                    <button class="btn btn-primary btn-sm mb-3 " id="printOD"><i class="fa-solid fa-print"></i></button>
                                </div>
                            </div>
                    ...
                    <div class="position-fixed bottom-0  w-100 z-index-3 card rounded-0">
                        <div class="row px-3 ">
                            ...
                            <div class="col-10  d-flex">
                                ...
                                <div class="pe-3 tab-div" id="all_tab">
                                    <button class="btn  rounded-0 ">All</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        @endif
            ...
            <script>
                ...
                const ajaxForStatusChange=(status,saleDetailId=0)=>{
                    $.ajax({
                ...
                const subComponent=(data,isUpdate=false)=>{
                    ...
                    let itemCount=0;
                    let saleDetails=data.sale_detail;
                    let updateStatus=isUpdate==true ? '<span class="badge badge-success fs-9 update_status">New Update</span>':'';
                    let fsd=saleDetails[0];
                    for (let i = 0; i < 5; i++) {
                        ...
                        if(sd && sd.product){
                            ...
                            items += `
                                <div class="d-flex justify-content-between align-items-center py-2">
                                    <span class="fw-semibold">${sd.product.name} ${sd.product_variation.variation_template_value ? '('+sd.product_variation.variation_template_value.name+')' : '' }</span>
                                    <span class="fw-bold">x ${Number(sd.quantity)} ${sd.uom.short_name}</span>
                                </div>
                        ...
                        <div class="card card-flush h-md-100 cursor-pointer bg-hover-light h-100"   >
                            <div class="card-header ribbon ribbon-top ribbon-vertical">
                                ...
                                <div class="w-100  d-flex justify-content-between mt-1 mb-10">
                                    <h6 class="fw-bold fs-6 time-count" id=""></h6>
                                    <h6 class="fw-bold fs-6">${updateStatus}</h6>
                                </div>
                            </div>
                ...
                const productItem=(sd)=>{
                    let product=sd.product;
                    let qty=sd.quantity;
                    let uom=sd.uom;
                    let status=sd.rest_order_status;
                    let detailNote=sd.note;
                    let note='';
                    if(!sd.product){
                        return '';
                    }
                    ...
                    return `
                            <div class="d-flex justify-content-between px-5 py-3 detailItem" type='button' data-id="${sd.id}">
                                <div class="">
                                    <h2 class="productLine fs-6 fw-bold productName ${status == `ready` ? `text-decoration-line-through text-gray-500` : ``}" >${product.name} ${sd.product_variation.variation_template_value ? '('+sd.product_variation.variation_template_value.name+')' : '' }</h2>
                                </div>
                                <div class="">
                                    <h2 class=" fs-6 fw-bold productQuantity">x ${Number(qty)} ${uom.short_name}</h2>
                                </div>
                            </div>
                            ${note}
                            <div class="separator separator-dashed"></div>
                ...
                $(document).on('click','.detailItem',function(){
                    let productName=$(this).find('.productName');
                    let detailId=$(this).data('id');
                    if(productName.hasClass('text-decoration-line-through')){
                        productName.removeClass('text-decoration-line-through text-gray-500');
                        let selectedSaleDetail=selectedOrder.sale_detail;
                        let saleDetailIndex=selectedSaleDetail.findIndex(sd=>sd.id==detailId);
                        selectedSaleDetail[saleDetailIndex].rest_order_status='preparing';
                        ...
                        productName.addClass('text-decoration-line-through text-gray-500');

                        let selectedSaleDetail=selectedOrder.sale_detail;
                        let saleDetailIndex=selectedSaleDetail.findIndex(sd=>sd.id==detailId);
                        selectedSaleDetail[saleDetailIndex].rest_order_status='ready';

                        let readySd=selectedSaleDetail.filter(s=>s.rest_order_status=='ready');
                        if(readySd.length >= selectedSaleDetail.length){
                            if(currentTag !='all'){
                ...
                $('#printOD').on('click',function() {
                    $.ajax({
                        url: `/order/display/print`,
                        type: 'get',
                        data:{
