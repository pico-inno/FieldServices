            ...
            <div class="row">
                ...
                <div class="col-md-9">
                    <div class="card card-p-4 card-flush" id="view2">
                        <div class="card-body">
                            <form action="" method="POST" id="updateGuestForm">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-md-3 mb-8">
                            ...
                            <div class="card">
                                ...
                                <div class="card-header card-header-stretch border-bottom border-gray-200">
                                    ...
                                    <div class="card-toolbar m-0">
                                        @php
                                        $folioInvoice = \Modules\Reservation\Entities\FolioInvoice::where('reservation_id', $currentReservation->id)->first();
                                        if($folioInvoice) {
                                        $folioInvoiceDetail = \Modules\Reservation\Entities\FolioInvoiceDetail::where('folio_invoice_id', $folioInvoice->id)->get();
                                        ...
                                        $sales = \App\Models\sale\sales::where('id', $sale_id)->get();
                                        }
                                        }
                                        }
                                        }
                                            ...
                                            <div class="col-md-6">
                                                <div class="table-responsive">
                                                    <table class="table align-middle table-row-dashed fw-bold text-gray-700 fs-7" id="allTabTotalAmounts">
                                                        @php
                                                        $saleAmount = isset($room_sale) && isset($sale) ? number_format(floatval($room_sale->sale_amount) + floatval($sale->sale_amount), 4, '.', '') : (isset($room_sale) ? number_format(floatval($room_sale->sale_amount), 4, '.', '') : (isset($sale) ? number_format(floatval($sale->sale_amount), 4, '.', '') : '0.0000'));
                                                        $totalItemDiscount = isset($room_sale) && isset($sale) ? number_format(floatval($room_sale->total_item_discount) + floatval($sale->total_item_discount), 4, '.', '') : (isset($room_sale) ? number_format(floatval($room_sale->total_item_discount), 4, '.', '') : (isset($sale) ? number_format(floatval($sale->total_item_discount), 4, '.', '') : '0.0000'));
                                                        $totalExtraDiscount = isset($sale) ? number_format(floatval($sale->extra_discount_amount), 4, '.', '') : '0.0000';
                                                        $totalSaleAmount = number_format(floatval($saleAmount) - (floatval($totalItemDiscount) + floatval($totalExtraDiscount)), 4, '.', '');
                                                        $paidAmount = isset($room_sale) && isset($sale) ? number_format(floatval($room_sale->paid_amount) + floatval($sale->paid_amount), 4, '.', '') : (isset($room_sale) ? number_format(floatval($room_sale->paid_amount), 4, '.', '') : (isset($sale) ? number_format(floatval($sale->paid_amount), 4, '.', '') : '0.0000'));
                                                        $balanceAmount = number_format(floatval($totalSaleAmount) - floatval($paidAmount), 4, '.', '');
                                                        @endphp
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>(-)</td>
                                                            <td>{{$totalExtraDiscount ?? '0.0000'}}</td>
                                                        </tr>
                                                        <tr>
                                                ...
                                                <div class="table-responsive">
                                                    <table class="table align-middle table-row-dashed fw-bold text-gray-700 fs-7" id="productBillingTotalTable">
                                                        ...
                                                        <tr>
                                                            <td>Total Item Discount</td>
                                                            <td>(-)</td>
                                                            <td>{{$sale->total_item_discount}}</td>
                                                        </tr>
                                                        <tr>
                                                            <td>Total Extra Discount</td>
                                                            <td>(-)</td>
                                                            <td>{{$sale->extra_discount_amount ?? '0.0000'}}</td>
                                                        </tr>
                                                        <tr>
                                                            <td>Total Sale Amount</td>
                                                            <td>(=)</td>
    ...
    function fetchMultipleReservationDetails(selectedReservations) {
        $.ajax({
            ...
            success: function(response) {
                ...
                var saleAmount = '0.0000';
                var totalItemDiscount = '0.0000';
                var totalExtraDiscount = '0.0000';
                var paidAmount = '0.0000';

                if (response.reservations) {
                    response.reservations.forEach(function(reservation) {
                        reservation.room_sales.forEach(function(room_sale) {
                            saleAmount = (parseFloat(saleAmount) + parseFloat(room_sale.sale_amount)).toFixed(4);
                            totalItemDiscount = (parseFloat(totalItemDiscount) + parseFloat(room_sale.total_item_discount)).toFixed(4);
                            paidAmount = (parseFloat(paidAmount) + parseFloat(room_sale.paid_amount)).toFixed(4);
                        });
                    })
                ...
                if (response.sales) {
                    response.sales.forEach(function(sale) {
                        saleAmount = (parseFloat(saleAmount) + parseFloat(sale.sale_amount)).toFixed(4);
                        totalItemDiscount = (parseFloat(totalItemDiscount) + parseFloat(sale.total_item_discount)).toFixed(4);
                        totalExtraDiscount = parseFloat(sale.extra_discount_amount).toFixed(4);
                        paidAmount = (parseFloat(paidAmount) + parseFloat(sale.paid_amount)).toFixed(4);
                    });
                }

                if (isNaN(parseFloat(totalExtraDiscount))) {
                    totalExtraDiscount = '0.0000';
                }

                if (isNaN(parseFloat(balanceAmount))) {
                    balanceAmount = '0.0000';
                }

                var totalSaleAmount = (parseFloat(saleAmount) - (parseFloat(totalItemDiscount) + parseFloat(totalExtraDiscount))).toFixed(4);
                var balanceAmount = (parseFloat(totalSaleAmount) - parseFloat(paidAmount)).toFixed(4);

                var saleAmoutRow = $('<tr></tr>');
                saleAmoutRow.append('<td>Sale Amount</td>');
                saleAmoutRow.append('<td>(=)</td>');
                saleAmoutRow.append('<td>' + saleAmount + '</td>');

                var totalDiscountRow = $('<tr></tr>');
                totalDiscountRow.append('<td>Total Item Discount</td>');
                totalDiscountRow.append('<td>(-)</td>');
                totalDiscountRow.append('<td>' + totalItemDiscount + '</td>');

                var extraDiscountRow = $('<tr></tr>');
                extraDiscountRow.append('<td>Total Extra Discount</td>');
                extraDiscountRow.append('<td>(-)</td>');
                extraDiscountRow.append('<td>' + totalExtraDiscount + '</td>');

                var totalSaleAmountRow = $('<tr></tr>');
                totalSaleAmountRow.append('<td>Total Sale Amount</td>');
                totalSaleAmountRow.append('<td>(=)</td>');
                totalSaleAmountRow.append('<td>' + totalSaleAmount + '</td>');

                var paidAmountRow = $('<tr></tr>');
                paidAmountRow.append('<td>Paid Amount</td>');
                paidAmountRow.append('<td>(-)</td>');
                paidAmountRow.append('<td>' + paidAmount + '</td>');

                var balanceAmountRow = $('<tr></tr>');
                balanceAmountRow.append('<td>Balance Amount</td>');
                balanceAmountRow.append('<td>(=)</td>');
                balanceAmountRow.append('<td>' + balanceAmount + '</td>');
                ...
                var productsaleAmount = '0.0000';
                var producttotalItemDiscount = '0.0000';
                var productpaidAmount = '0.0000';
                var producttotalExtraDiscount = '0.0000';

                $('#productBillingTotalTable tbody').empty();
                if (response.sales) {
                    // console.log('sale total', response.sales);
                    response.sales.forEach(function(sale) {
                        productsaleAmount = (parseFloat(productsaleAmount) + parseFloat(sale.sale_amount)).toFixed(4);
                        producttotalItemDiscount = (parseFloat(producttotalItemDiscount) + parseFloat(sale.total_item_discount)).toFixed(4);
                        producttotalExtraDiscount = parseFloat(sale.extra_discount_amount).toFixed(4);
                        productpaidAmount = (parseFloat(productpaidAmount) + parseFloat(sale.paid_amount)).toFixed(4);
                    });
                }

                if (isNaN(parseFloat(producttotalExtraDiscount))) {
                    producttotalExtraDiscount = '0.0000';
                }

                if (isNaN(parseFloat(productbalanceAmount))) {
                    productbalanceAmount = '0.0000';
                }

                var producttotalSaleAmount = (parseFloat(productsaleAmount) - (parseFloat(producttotalItemDiscount) + parseFloat(producttotalExtraDiscount))).toFixed(4);
                var productbalanceAmount = (parseFloat(producttotalSaleAmount) - parseFloat(productpaidAmount)).toFixed(4);

                var productsaleAmoutRow = $('<tr></tr>');
                productsaleAmoutRow.append('<td>Sale Amount</td>');
                productsaleAmoutRow.append('<td>(=)</td>');
                productsaleAmoutRow.append('<td>' + (productsaleAmount ? productsaleAmount : '0.0000') + '</td>');
                ...
                producttotalDiscountRow.append('<td>' + producttotalItemDiscount + '</td>');
                ...
                producttotalExtraDiscountRow.append('<td>' + producttotalExtraDiscount + '</td>');
