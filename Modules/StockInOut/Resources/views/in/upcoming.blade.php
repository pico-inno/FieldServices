...
@section('title')
    <!--begin::Heading-->
    <h1 class="text-dark fw-bold my-0 fs-2">{{__('stockinout::stockin.upcoming_list')}}</h1>
    <!--end::Heading-->
    <!--begin::Breadcrumb-->
    <ul class="breadcrumb fw-semibold fs-base my-1">
        <li class="breadcrumb-item text-muted">{{__('stockinout::stockin.stockin')}}</li>
        <li class="breadcrumb-item text-dark">{{__('stockinout::stockin.upcoming_list')}}</li>
    </ul>
    <!--end::Breadcrumb-->
@endsection
@section('content')
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title">
                                <h2>{{__('stockinout::stockin.filters')}}</h2>
                            </div>
                        </div>
                    ...
                    <div class="card-title">
                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <input type="text" data-kt-customer-table-filter="search"
                                   ...
                                   placeholder="{{__('stockinout::stockin.search_purchase_order')}}"/>
                        </div>
                        <!--end::Search-->
                    </div>
                    <!--begin::Card title-->
                ...
                <div class="card-body pt-0">
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="stockin_table">
                        ...
                        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                            <th class="min-w-100px">{{__('stockinout::stockin.actions')}}</th>
                            <th class="min-w-125px">{{__('stockinout::stockin.order_date')}}</th>
                            <th class="min-w-125px">{{__('stockinout::stockin.voucher_no')}}</th>
                            <th class="min-w-125px">{{__('stockinout::stockin.location')}}</th>
                            <th class="min-w-125px">{{__('stockinout::stockin.order_person')}}</th>
                            <th class="min-w-125px">{{__('stockinout::stockin.status')}}</th>
                            <th class="min-w-125px">{{__('stockinout::stockin.received_qty')}}</th>
                        </tr>
                        <!--end::Table row-->
                        </thead>
                        <!--end::Table head-->
                        ...
                        @if (count($upcomingLists) === 0)
                            <tr class="text-gray-400">
                                <td colspan="7" class="text-center">{{__('stockinout::stockin.no_data_table')}}</td>
                            </tr>
                        @else
                                                @foreach($upcomingLists as $key => $upcoming)
                                                    <tr>
                                                        <!--begin::Action=-->
                                                        <td>
                                                            <a href="{{route('stock-in.receive',$upcoming->id)}}" class="btn btn-sm btn-primary">Receive</a>
                                                        </td>
                                                        <!--end::Action=-->
                                                        <!--begin::Name=-->
                                                        <td>
                                                            <a href="../../demo7/dist/apps/ecommerce/customers/details.html"
                                                               class="text-gray-800 text-hover-primary mb-1">{{$upcoming->purchased_at}}</a>
                                                        </td>
                                                        <!--end::Name=-->
                                                        <!--begin::Email=-->
                                                        <td>
                                                            <a href="#"
                                                               class="text-gray-600 text-hover-primary mb-1">{{$upcoming->purchase_voucher_no}}</a>
                                                        </td>
                                                        <td>
                                                            <a href="#"
                                                               class="text-gray-600 text-hover-primary mb-1">{{$upcoming->location_name}}</a>
                                                        </td>
                                                        <td>
                                                            <a href="#"
                                                        ...
                                                        <td>
                                                            @switch($upcoming->status)
                                                                @case('order')
                                                                    <div class="badge badge-light-primary">{{$upcoming->status}}</div>
                                                                    @break

                                                                @case('partial')
                                                                    <div class="badge badge-light-warning">{{$upcoming->status}}</div>
                                                                    @break
                                                                @default
                                                                    <div class="badge badge-light">{{$upcoming->status}}</div>
                                                            @endswitch
                                                        </td>
                                                        <td>
                                                            <a href="#"
                                                               class="badge  @if($upcoming->total_received_quantity == $upcoming->total_quantity)
                                                                    badge-light-success
                                                                @elseif($upcoming->total_received_quantity == 0)
                                                                    badge-light-danger
                                                                @else
                                                                    badge-light-dark
                                                                @endif fs-6 text-hover-primary mb-1">
                                                                {{number_format($upcoming->total_received_quantity, 2)}}
                                                            </a>
                                                        </td>
                                                    </tr>
                                                @endforeach
